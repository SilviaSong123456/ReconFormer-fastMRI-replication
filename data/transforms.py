...
def fft2(data, normalized=True):
    ...
    Input and output store complex values as two real-valued channels
    in the final dimension: [..., H, W, 2].
    """
    assert data.size(-1) == 2

    # Move the k-space/image center before FFT.
    data = ifftshift(data, dim=(-3, -2))

    # Convert [..., H, W, 2] float representation to complex tensor.
    data = torch.view_as_complex(data.contiguous())

    # Apply FFT across H and W.
    data = torch.fft.fft2(
        data,
        dim=(-2, -1),
    ...
    # Convert complex tensor back to [..., H, W, 2] float representation.
    data = torch.view_as_real(data)

    # Restore centered representation.
    data = fftshift(data, dim=(-3, -2))
...
def ifft2(data, normalized=True):
    ...
    # Move the k-space center before inverse FFT.
    data = ifftshift(data, dim=(-3, -2))

    # Convert [..., H, W, 2] float representation to complex tensor.
    data = torch.view_as_complex(data.contiguous())

    # Apply inverse FFT across H and W.
