...
def test_recon_save(net_g, data_loader, args):
    ...
    with torch.no_grad():
        ...
        for fname in tqdm(outputs):
            output = np.stack([
                out
                for _, out in sorted(outputs[fname])
            ])

            target = np.stack([
                tgt
                for _, tgt in sorted(targets[fname])
            ])

            input = np.stack([
                inp
                for _, inp in sorted(inputs[fname])
            ])
        ...
        # Calculate averages across volumes
        metrics = {
        ...
        print("\nImprovement of ReconFormer over Zero-filled:")
        print({
            "nmse_absolute_reduction":
                zero_filled_metrics["nmse"]
                - metrics["nmse"],
            ...
            "ssim_absolute_improvement":
                metrics["ssim"]
                - zero_filled_metrics["ssim"],

            "psnr_improvement_db":
                metrics["psnr"]
                - zero_filled_metrics["psnr"],
        })
